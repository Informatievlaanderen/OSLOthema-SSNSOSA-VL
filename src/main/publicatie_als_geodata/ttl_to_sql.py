...
DEFAULT_TTL    = SCRIPT_DIR / "../input/r-factor/rfactor.trig"
...
# Leeg voor SSNSOSA: SOSA-klassen worden rechtstreeks als class_uri gebruikt.
...
RELATION_TO_PREDICATE = {
    # sosa:hasMember: collectie (source) → observatie (target), forward
    "heeft_lid_observatie": ("http://www.w3.org/ns/sosa/hasMember", "forward"),
}

# Voor disambiguatie van JOIN-relaties: welke doelklasse verwachten we? (leeg voor SSNSOSA)
RELATION_TARGET_CLASS: dict[str, str] = {}

# Multi-hop property paths: (tabel_naam, kolom) → [pred1, pred2, ...]
# Elk element is een string (URI) of een lijst van alternatieve URI's.
# De traverse volgt het pad stap voor stap; de laatste stap levert een Literal of URI.
MULTI_HOP_PREDS: dict[tuple[str, str], list] = {
    # observatie.begin_tijd: obs → phenomenonTime → interval → hasBeginning → instant → inXSDDate/DateTime
    ("observatie", "begin_tijd"): [
    ...
    # observatie.eind_tijd: obs → phenomenonTime → interval → hasEnd → instant → inXSDDate/DateTime
    ("observatie", "eind_tijd"): [
        "http://www.w3.org/ns/sosa/phenomenonTime",
    ...
    # meetstation.geom_wkt: station → hasGeometry → geometrie-node → asWKT
    ("meetstation", "geom_wkt"): [
        "http://www.opengis.net/ont/geosparql#hasGeometry",
        "http://www.opengis.net/ont/geosparql#asWKT",
    ],
}

# Klasse-uitsluiting per tabel: subjects die ook tot een van deze klassen behoren
# worden uitgesloten. Nodig omdat sosa:Platform ook sosa:FeatureOfInterest is.
TABLE_CLASS_EXCLUDE: dict[str, frozenset[str]] = {
    "studiegebied": frozenset({
        "http://www.w3.org/ns/sosa/Platform",
        "http://www.w3.org/ns/sosa/SpatialSample",
    }),
}
...
def load_graph(ttl_path: Path) -> Graph:
    ...
    g = Graph()
    g.parse(str(ttl_path))  # rdflib detecteert formaat via extensie (.ttl, .trig, …)
...
def traverse_path(g: Graph, subject: URIRef, path: list) -> str:
    """Doorloopt een pad van predicaten vanaf subject en geeft SQL-waarde terug.

    Elk element in path is een string (URI) of een lijst van alternatieve URI's.
    De traverse probeert alle alternatieven en verzamelt alle bereikbare nodes.
    Geeft de eerste gevonden Literal of URI terug als SQL-waarde, anders 'NULL'.
    """
    current: set = {subject}
    for step in path:
        preds = [step] if isinstance(step, str) else step
        nxt: set = set()
        for node in current:
            if not isinstance(node, URIRef):
                continue
            for pred_uri in preds:
                for obj in g.objects(node, URIRef(pred_uri)):
                    nxt.add(obj)
        current = nxt
        if not current:
            return "NULL"
    for node in sorted(current, key=str):
        if isinstance(node, Literal):
            return coerce_literal(node)
        if isinstance(node, URIRef):
            return sql_escape(str(node))
    return "NULL"
...
def get_value(
    ...
    multi_key = (table_name, col)
    if multi_key in MULTI_HOP_PREDS:
        return traverse_path(g, subject, MULTI_HOP_PREDS[multi_key])
...
def generate_regular(
    ...
    subjects = class_subjects.get(table.class_uri, set())
    # Sluit subjects uit die ook tot een andere klasse behoren (bijv. Platform uit FOI)
    exclude_classes = TABLE_CLASS_EXCLUDE.get(table.name, frozenset())
    if exclude_classes:
        excluded = set().union(*(class_subjects.get(ec, set()) for ec in exclude_classes))
        subjects = subjects - excluded
